...
*I like the choice to include the original instructions, makes the comments really clear on the step-by-step*

*Do you like the simplicity of the entire challenge being in one chunk? My hesitation is that if you're looking for one part ie. just how to use strsplit(), you're gonna have to dig a bit since it's not all outlined with subheadings if that makes sense*
-AJL
```{r}
...
```
*small note but I would keep naming the chunks with the challenge just for consistency's sake*

*Why for the second one do you assign the rows to r and then use r to extract rows? Maybe explain that choice/that it's an option since you didn't do that for the first one with the columns - also explaining what drop=FALSE means might be a good idea as I didn't have to do that for my extractions*
*for the last, you're only extracting the 9th column in the first option and then ony the 2nd row for the second option - I think what you want is rows 2-6, columns 2-9 so you could do something like m[c(2:6), c(2:9)]*
-AJL
...
d
```
*maybe explain that the second half is extraction/subsetting since there's no instructions or comments there*
-AJL
...
```
*re: your question about stacking - that's what happened in my code as well!*
-AJL
...
*for coercing, if this is helpful I did df[,2]<-as.factor(df[,2])
so, I used brackets to extract*

*Your trick with the $ might still work but I wouldn't use the logic/factor before <- because you're turning them into vectors which I don't think was your intention*
-AJL
...
*I love the idea of keeping these errors and workarounds saved somewhere! Super smart since you might fall into the same traps again and again. For the record, none of the code broke on my run through! Great job!*
-AJL

*After looking at the knitted version:*

*Title part is very pretty but the rest is quite simple in comparison*

*I would recommend putting some subheading outside of the chunks with the pound signs in the rmarkddown text just because all the chunks kinda blend together as you scroll down the page*
-Best, AJL
